...
const flipRotate = (input, max = 360) => {
...
class brush {
  ...
  image (
    ...
    newx ??= 0; newy ??= 0; neww ??= image.width; newh ??= image.height;
		...
		let { tint, flip, rotation, rotationX, rotationY, center = {}, debug = false } = options;
    ...
    if (rotationX) c.scale(flipRotate(rotationX, 360), 1);

    if (rotationY) c.scale(1, flipRotate(rotationY, 360));
  ...
  gradient = (values) => {
    const { ctx } = this;
    const gradient = ctx.createLinearGradient(values.gx || 0, values.gy || 0, values.gx1 || 0, values.gy1 || 0);
